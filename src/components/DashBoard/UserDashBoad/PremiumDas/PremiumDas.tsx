...
const PremiumDas: React.FC = () => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const met = {
      ...
      link: "",
      email: person?.email,
      meetingReqTime: moment().format("MMMM Do YYYY, h:mm:ss a"),
      ReplyTime: null,
    };
  ...
  const deleteMutation = useMutation({
    mutationFn: async (id: string) => {  
      const res = await axiosPrivate.delete(`/setMeeting/${id}`);
      return res.data;
    },
    onSuccess: () => {
      refetch(); 
  ...
  return (
    <div className="min-h-screen flex flex-col bg-gradient-to-b from-gray-50 to-gray-100 px-3 py-6 sm:px-4 sm:py-8 md:px-6 md:py-10 lg:px-8 lg:py-12 font-sans">
      ...
      <div className="w-full max-w-full sm:max-w-md md:max-w-2xl lg:max-w-5xl mx-auto mb-6 sm:mb-8">
        {(data?.Premium === false || (data && !("Premium" in data))) && (
          <div className="bg-gradient-to-r from-amber-50 to-white border border-amber-200 rounded-xl p-4 sm:p-5 shadow-sm relative overflow-hidden">
            ...
            <div className="flex flex-col sm:flex-row gap-4 sm:items-center justify-between relative z-10">
              ...
              <button
                ...
                className="group whitespace-nowrap bg-amber-900 hover:bg-amber-800 text-amber-50 text-sm font-semibold py-2.5 px-5 rounded-lg transition-all shadow-md hover:shadow-lg flex items-center justify-center gap-2"
              >
                Get Membership
            ...
            {!meetings || meetings.length === 0 ? (
              ...
              <div className="space-y-3 sm:space-y-4 md:space-y-5 max-h-[500px] overflow-y-auto pr-2 custom-scrollbar">
                {meetings.map((meeting: any, index: number) => (
                  <div
                    key={meeting._id || index} // Better to use _id if available
                    className="group/card border border-gray-100 rounded-lg sm:rounded-xl p-3 sm:p-4 md:p-5 bg-white hover:bg-amber-50/30 hover:border-amber-200 transition-all duration-300 shadow-sm hover:shadow-md relative"
                  >
                    {/* Meeting Topic & Actions (Status + Delete) */}
                    <div className="flex flex-col sm:flex-row sm:items-start justify-between mb-3 gap-2">
                      ...
                      {/* ACTION GROUP: Status Badge + Delete Button */}
                      <div className="flex items-start gap-2 self-start">
                        {/* Status Badge */}
                        <div
                          className={`px-2 sm:px-3 py-1 rounded-full text-[10px] sm:text-xs font-bold border shadow-sm flex items-center gap-1 whitespace-nowrap ${
                            meeting.ReplyTime
                        ...
                        <button
                          ...
                          className="p-1.5 rounded-lg text-gray-400 hover:text-red-500 hover:bg-red-50 transition-all duration-200 border border-transparent hover:border-red-100"
                        >
                    ...
                    <div className="bg-gray-50 rounded p-2.5 sm:p-3 md:p-4 mb-3 border border-gray-100">
                      <p className="text-[10px] sm:text-xs text-amber-600 font-bold uppercase tracking-wider mb-1 sm:mb-2">
                        Agenda
                      </p>
