...
const EnrolledCourses = () => {
  ...
  // State for the Modal
  const [selectedCourse, setSelectedCourse] = useState<OrderDataType | null>(null);
  ...
  if (isLoading) {
    return (
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 p-6 max-w-[1400px] mx-auto">
        {[...Array(4)].map((_, i) => (
          <div key={i} className="bg-white rounded-2xl border border-slate-200 shadow-sm h-[380px] animate-pulse">
  ...
  return (
    <div className="max-w-[1400px] mx-auto px-4 md:px-8 py-10">
      ...
      <div className="mb-8 flex flex-col sm:flex-row sm:items-end justify-between gap-4">
        <div>
          <h2 className="text-2xl md:text-3xl font-bold text-slate-900">My Learning</h2>
      ...
      {!data || data.length === 0 ? (
        ...
        <div className="flex flex-col items-center justify-center py-20 bg-slate-50 rounded-3xl border-2 border-dashed border-slate-200 text-center mx-auto max-w-2xl">
          ...
          <h3 className="text-xl font-bold text-slate-900">No courses enrolled yet</h3>
          <p className="text-slate-500 max-w-xs md:max-w-md mt-2 mb-6">
            Start your journey today. Browse our catalog to find the perfect course for you.
          </p>
            ...
            >
              ...
              <div className="p-5 flex flex-col flex-grow">
                <div className="flex items-center gap-2 text-xs font-medium text-slate-400 mb-3">
                  ...
                  <span>Enrolled: {course.orderDate}</span>
                </div>
            ...
            <div className="p-6 md:p-8">
              ...
              <div className="flex items-center gap-4 text-sm text-slate-500 mb-6">
                 <span className="flex items-center gap-1">
                    <Calendar className="w-4 h-4" /> {selectedCourse.orderDate}
                 </span>
                 <span className="w-1 h-1 bg-slate-300 rounded-full"></span>
                 <span>Status: {selectedCourse.deliveryStatus}</span>
               
              </div>
              <div className=" text-xl font-bold mb-6">
                  <span>Drive Link : <Link to={selectedCourse.drive}><span className="text-indigo-600">Click to Learn</span></Link></span>
              </div>
              ...
              <div className="space-y-3">
                ...
                <Link to={`/d/${selectedCourse.courseId}`}>
                <button 
                  onClick={closeModal}
